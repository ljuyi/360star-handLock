...
for(var i = 0; i < oLi.length; i++){
    oLi[i].addEventListener('touchstart', touch, false);
    oLi[i].addEventListener('touchmove', touch, false);
    oLi[i].addEventListener('touchend', touch, false);
    oLi[i].index = i;                                    /*给每个点设置唯一索引*/
    oLi[i].x = oLi[i].offsetLeft;                        /*给每个点设置x,y值为其在页面上的位置*/
    oLi[i].y = oLi[i].offsetTop;
}

oLabel[0].onclick = function(){
    route.isCheck = false;
    initPoint();
}
oLabel[1].onclick = function(){
    route.isCheck = true;
    initPoint();
}

localStorage.setItem('psd',[0,1,2,4,6,7,8]);               /*初始密码*/
...
function touch(event){                                            /*触摸事件*/
    ...
    switch(event.type){
        ...
        case 'touchend':
            if(route.isCheck){
                if(route.input.toString() == localStorage.getItem('psd').toString()){
                    info.innerHTML = '<p>密码正确!</p>';
                    SRouter.changeUrl('selectRoom')
                }else{
                ...
                if(route.input.length < 5){
                    ...
                    if(firstSet.toString() == route.input.toString()){
                        ...
                        localStorage.setItem('psd', route.input);
                        firstSet = [];
                        initPoint();
                    }else{
